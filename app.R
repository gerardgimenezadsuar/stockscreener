    ...
    av_ni <- filtered_table() %>%
      ...
      reactable(filtered_table() %>% select(-weights),
              ...
              columns = list(
                ...
                revenue = colDef(name = "Revenue (in Millions)", format = reactable::colFormat(currency = "USD",
                                                                             separators = TRUE),
                ...
                OperatingIncomeLoss = colDef(name = "Operating Income (in Millions)", format = reactable::colFormat(currency = "USD",
                                                                                              separators = TRUE),
                                 footer = function(values) paste0("$", formatC(sum(values, na.rm=T), format="f", digits=2, big.mark=","), "  (", formatC(sum(!is.na(values)), format="f", digits=0, big.mark=","), ")")),
                
                net_income = colDef(name = "Net Income (in Millions)", format = reactable::colFormat(currency = "USD",
                                                                             separators = TRUE),
                                       footer = function(values) paste0("$", formatC(sum(values, na.rm=T), format="f", digits=2, big.mark=","), "  (", formatC(sum(!is.na(values)), format="f", digits=0, big.mark=","), ")")),
